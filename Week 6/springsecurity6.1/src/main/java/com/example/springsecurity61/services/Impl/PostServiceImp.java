package com.example.springsecurity61.services.Impl;

import com.example.springsecurity61.dto.PostDto;
import com.example.springsecurity61.entity.Post;
import com.example.springsecurity61.exception.ResourceNotFoundException;
import com.example.springsecurity61.repository.PostRepository;
...
public class PostServiceImp implements PostService {
    ...
    @Override
    public List<PostDto> getAllPost(Integer  pageNo , String sorBy, String direction) {
        Sort.Direction sortDirection = direction.equalsIgnoreCase("ASC")
                ? Sort.Direction.ASC
                : Sort.Direction.DESC;


        Pageable pageable = PageRequest.of(pageNo,PAGE_SIZE,Sort.by(sortDirection,sorBy));


        Page<Post> pages = postRepository.findAll(pageable);

        return pages.stream().map((element) -> modelMapper.map(element, PostDto.class)).toList();
    }
    ...
    public PostDto partialUpdate(Long id, Map<String, Objects> updates) {
        ...
        Post postToUpdate = postRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("Not found"));
        ...
        updates.forEach((key,value)->{
            ...
            ReflectionUtils.setField(field,postToUpdate,value);
